...
def test_route_passing_pet_id():
    ...
    http_request = HttpRequest(view_arg={"pet_id": faker.random_number(digits=2)})

    response = find_pet_controller.route(http_request)

    # Testing inputs
    assert (
        find_pet_use_case.by_pet_id_param["pet_id"] == http_request.view_arg["pet_id"]
    )
...
def test_route_passing_user_id():
    ...
    find_pet_use_case = FindPetSpy(PetRepositorySpy())
    find_pet_controller = FindPetController(find_pet_use_case)
    http_request = HttpRequest(view_arg={"user_id": faker.random_number(digits=5)})
